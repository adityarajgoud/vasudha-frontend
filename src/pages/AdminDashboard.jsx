...
import { UploadCloud, FileText, CheckCircle2, Clock, XCircle, AlertTriangle, Plus, X } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      ...
      setUploadSuccess('Dataset submitted successfully! Pending Super Admin review.');
      setTitle('');
      setFile(null);
      fetchMyDatasets();
      setTimeout(() => {
      ...
      setUploadError(err.response?.data?.message || 'Upload failed. Ensure CSV schema matches the chart type.');
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-4 pb-6 mb-8 border-b sm:flex-row sm:items-center sm:justify-between border-slate-200">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Manage and submit environmental and energy datasets for review.</p>
        </div>
        <button
      ...
      <div className="overflow-hidden bg-white border shadow-sm rounded-xl border-slate-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <h2 className="text-sm font-bold tracking-wide uppercase text-slate-800">Submitted Datasets</h2>
          <span className="text-xs font-medium text-slate-500">{datasets.length} Total Submissions</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">No datasets submitted yet.</p>
            <p className="mt-1 text-xs text-slate-400">Click "Add New Dataset" to upload your first CSV.</p>
          </div>
        ) : (
                ...
                {datasets.map((item) => (
                  <tr key={item._id} className="transition-colors hover:bg-slate-50/80">
                    ...
                    <td className="px-6 py-4 font-mono text-xs text-slate-600">
                      {item.chartType === 'lat_long_map' && 'India Map (Lat/Long)'}
                      {item.chartType === 'state_heatmap' && 'State Heatmap'}
                      {['line', 'bar', 'area'].includes(item.chartType) && `Time Series (${item.chartType.toUpperCase()})`}
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-500">{item.data?.length || 0}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="w-full max-w-lg p-6 bg-white border shadow-2xl rounded-2xl border-slate-100">
            <div className="flex items-center justify-between pb-4 mb-5 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-800">Add New Dataset</h3>
              <button
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              ...
              <div className="p-3 text-xs border rounded-lg bg-slate-50 border-slate-200 text-slate-600">
                <span className="font-semibold text-slate-700">Expected CSV Schema: </span>
                {chartType === 'lat_long_map' && 'Requires Latitude, Longitude, and a metric column.'}
                {chartType === 'state_heatmap' && 'Requires State and Value columns.'}
                {['line', 'bar', 'area'].includes(chartType) && 'Requires Date/Year and at least one numeric metric column.'}
              </div>
